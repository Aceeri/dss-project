...
use gfx_hal::{buffer, command, display, format as f, format::{AsFormat, ChannelType, Rgba8Srgb as ColorFormat, Swizzle}, image as i, memory as m, pass, pass::Subpass, pool, prelude::*, pso, pso::{PipelineStage, ShaderStageFlags, VertexInputRate}, queue::QueueGroup, window::{self, Extent2D}};
    ...
    B: gfx_hal::Backend,
{
    pub fn new(
        ...
        let sparsely_bound = physical_device
            .features()
            .contains(gfx_hal::Features::SPARSE_BINDING | gfx_hal::Features::SPARSE_RESIDENCY_IMAGE_2D);
        let mut gpu = unsafe {
            physical_device
                .open(
                    &[(family, &[1.0])],
        ...
        // Image
        let img_data = include_bytes!("./resources/logo.png");
        ...
        let pipeline = {
            let vs_module = {
                let spirv =
                    gfx_auxil::read_spirv(Cursor::new(&include_bytes!("./resources/quad.vert.spv")[..]))
                        .unwrap();
                unsafe { device.create_shader_module(&spirv) }.unwrap()
            };
            let fs_module = {
                let spirv =
                    gfx_auxil::read_spirv(Cursor::new(&include_bytes!("./resources/quad.frag.spv")[..]))
                        .unwrap();
                unsafe { device.create_shader_module(&spirv) }.unwrap()
            };
